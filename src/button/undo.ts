...
abstract class Collection<T> {
  ...
  const undoStep = (): string => {
    ...
    const cell = document.querySelector(`[row="${arr[0]}"][column="${arr[1]}"]`);
    console.log(`undid row ${arr[0]+1} column ${arr[1]+1} from ${cell.innerHTML} to ${arr[2]}`);
    if(arr[2] === 0) cell.innerHTML = ``;
    else cell.innerHTML = arr[2].toString();
